...
# TODO(user): Remove this override when the guard is removed altogether.
mox.Mox.RAISE_SWALLOWED_EXCEPTIONS = True
...
def IgnoreSwallowedExceptions(func):
  """Call func, ignoring swallowed exceptions."""
  old_setting = mox.Mox.RAISE_SWALLOWED_EXCEPTIONS
  mox.Mox.RAISE_SWALLOWED_EXCEPTIONS = False
  try:
    func()
  finally:
    mox.Mox.RAISE_SWALLOWED_EXCEPTIONS = old_setting


class PickleExceptions(unittest.TestCase):
  """Test pickling and unpickling of mox exceptions."""

  def testPickleAndUnpickle(self):
    exceptions_with_args = {
        mox.Error: (),
        mox.ExpectedMethodCallsError: (
            [mox.MockMethod("method", [], [], True)],),
        mox.UnexpectedMethodCallError: (
            mox.MockMethod("method", [], [], True),
            mox.UnorderedGroup("group", [])),
        mox.UnknownMethodCallError: ("method",),
        mox.PrivateAttributeError: ("attribute",),
        mox.SwallowedExceptionError: ([mox.Error()],),
        # ExpectedMockCreationError actually wants a sequence of MockObjects,
        # but MockObjects themselves don't pickle well (if at all).
        mox.ExpectedMockCreationError: ([object()],),
        mox.UnexpectedMockCreationError: (object,),
        }

    for cls_obj, args in exceptions_with_args.items():
      pickled_string = pickle.dumps(cls_obj(*args))
      unpickled_value = pickle.loads(pickled_string)
      self.assertTrue(isinstance(unpickled_value, cls_obj))
...
class ExpectedMethodCallsErrorTest(unittest.TestCase):
  ...
  def testOneError(self):
    ...
    self.assertRegexpMatches(str(e),
        r"Verify: Expected methods never called:\n"
        r"  0.  testMethod\(1, 2\) -> 'output'"
        r" \[Expectation set at: mox_test.py:\d+\]")
  ...
  def testManyErrors(self):
    ...
    self.assertRegexpMatches(str(e),
        ...
        r"  0.  testMethod\(1, 2\) -> 'output'"
        r" \[Expectation set at: mox_test.py:\d+\]\n"
        r"  1.  testMethod\(a=1, b=2, c='only named'\) -> None"
        r" \[Expectation set at: mox_test.py:\d+\]\n"
        r"  2.  testMethod2\(\) -> 44"
        r" \[Expectation set at: mox_test.py:\d+\]\n"
        r"  3.  testMethod\(1, 2\) -> 'output'"
        r" \[Expectation set at: mox_test.py:\d+\]")
...
class RegexTest(unittest.TestCase):
  ...
  def testReprWithFlags(self):
    ...
    # Test with re.UNICODE | re.IGNORECASE (which happens to be 34) which is
    # a valid combination of flags for string regular expressions in any
    # version of Python.
    self.assertTrue(repr(mox.Regex(r"a\s+b", flags=34)) ==
                    "<regular expression 'a\s+b', flags=34>")
...
class MockAnythingTest(unittest.TestCase):
  ...
  def testReplayWithUnexpectedCall(self):
    ...
    self.mock_object.ValidCall()          # setup method call
    self.mock_object._Replay()             # start replay mode
    self.assertRaisesRegexp(mox.UnexpectedMethodCallError,
                            'Expectation set at: mox_test.py',
...
class MethodCheckerTest(unittest.TestCase):
  ...
  def testNoParameters(self):
    method = mox.MockMethod('NoParameters', [], [], False,
                            ...
                            method_class=CheckCallTestClass)
    method()
...
class MockObjectTest(unittest.TestCase):
  ...
  def testGetAttr(self):
    """Code should be able to call getattr() on a mock object."""
    self.assertTrue(getattr(self.mock_object, 'ValidCall'))
    self.assertFalse(getattr(self.mock_object, 'InvalidCall', None))
  ...
  def testMockChildContains_ExpectedContains_Success(self):
    """Test that __contains__ gets mocked in Dummy of child.

    In this test, _Verify() succeeds.
    """
    dummy = mox.MockObject(ChildClass)
    dummy.__contains__('X').AndReturn(True)
  ...
  def testCreatingFromFunctionCreatesCorrectDescription(self):
    myobj = mox.MockObject(TestFunction)
    self.assertEqual(myobj._description, 'TestFunction')

  def testCreatingFromMethodCreatesCorrectDescription(self):
    myobj = mox.MockObject(TestClass.MethodWithArgs)
    self.assertEqual(myobj._description, 'TestClass.MethodWithArgs')
...
class MoxTest(unittest.TestCase):
  ...
  def testVerification(self):
    """Test verification via a context handler."""
    mock_obj = self.mox.CreateMock(TestClass)
    mock_obj.ValidCall().AndReturn("yes")
  ...
  def testVerificationPropagatesExceptions(self):
    def TestHelper():
      mock_obj = self.mox.CreateMock(TestClass)
      mock_obj.ValidCall().AndReturn("yes")
      with self.mox.Verification():
        # Explicitly don't call the expected call.
        raise ValueError()

    self.assertRaises(ValueError, TestHelper)
    # We also need to check that VerifyAll was not called.  Do this by
    # calling it now, and checking that we get the expected error.
    self.assertRaises(mox.ExpectedMethodCallsError, self.mox.VerifyAll)
  ...
  def testStubOut_SignatureMatching_init_(self):
    ...
    # Create an instance of a child class, which calls the parent __init__
  ...
  def testStubOutClass_OldStyle(self):
    ...
    if _IS_PYTHON_3:
      return  # Old style classes do not exist in Python 3
  ...
  def testStubOutClassConstructor(self):
    self.mox.StubOutWithMock(mox_test_helper, 'ExampleClass')
    some_obj = object()
    mox_test_helper.ExampleClass().AndReturn(some_obj)
    self.mox.ReplayAll()
  ...
  def testStubOutClassTooManyCreated(self):
    ...
    mox_test_helper.CallableClass(1, 2, nine=9)
  ...
  def testUnboundModuleFunctionVerification(self):
    mock = self.mox.CreateMock(mox_test_helper)
    mock.MyTestFunction(1, 2)
    self.mox.ReplayAll()
    mock.MyTestFunction(1, 2)
    self.mox.VerifyAll()
  ...
  def testSwallowedUnknownMethodCall(self):
    ...
    self.assertRaises(mox.SwallowedExceptionError, self.mox.VerifyAll)
    IgnoreSwallowedExceptions(self.mox.VerifyAll)
