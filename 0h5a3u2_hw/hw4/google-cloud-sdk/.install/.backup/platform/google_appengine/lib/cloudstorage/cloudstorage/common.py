...
# Metadata that can be returned from stat call.
_GCS_METADATA = ['x-goog-meta-',
                 'content-disposition',
                 'cache-control',
                 'content-encoding']
# Options user can specify when creating a file.
_GCS_OPTIONS = _GCS_METADATA + ['x-goog-acl']
# The XML namespace returned by CS response.
CS_XML_NS = 'http://doc.s3.amazonaws.com/2006-03-01'
# Endpoint path on local dev appserver
LOCAL_GCS_ENDPOINT = '/_ah/gcs'
# See set_access_token function.
_access_token = ''


# Max number of results from get bucket according to GCS doc.
...
# For backwards compatibility with old naming
...
# Python 25 doesn't support parsing microseconds.
...
def local_run():
  ...
  if 'remote_api' in os.environ.get('SERVER_SOFTWARE', ''):
    return False
  return os.environ.get('GAE_ENV') in [
      'localdev',  # devappserver2, testbed, and RGE's setting
      None,  # unit tests not using testbed
  ]
...
# Tags with namespaces for XML parsing.
