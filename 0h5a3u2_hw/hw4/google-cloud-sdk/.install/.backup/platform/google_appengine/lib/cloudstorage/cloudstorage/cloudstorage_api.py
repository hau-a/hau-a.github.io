...
def open(filename,
  ...
  Args:
    ...
    mode: 'rb' for reading mode. 'wb' for writing mode.
  ...
  api = storage_api._get_storage_api(
      retry_params=retry_params, token_params=token_params)
  filename = api_utils._quote_filename(filename)

  if mode in ('r', 'w'):
    logging.warning(
        'Cloudstorage supports only binary modes, modes "r" and "w" are '
        'deprecated and automatically switched to "rb" and "wb".')
    mode += 'b'
...
def delete(filename, retry_params=None, _account_id=None, token_params=None):
  """Delete a Google Cloud Storage file.

  Args:
    filename: A Google Cloud Storage filename of form '/bucket/filename'.
    retry_params: An api_utils.RetryParams for this call to GCS. If None, the
      default one is used.
    _account_id: deprecated.
    token_params: A dict of parameters that can be used with a custom token
      maker. When no custom token maker is configured, this is unused.
  ...
  """
  delete_async(filename, retry_params, token_params).get_result()


@ndb.tasklet
def delete_async(filename, retry_params=None, token_params=None):
  """Delete a Google Cloud Storage file.
  ...
  common.validate_file_path(filename)
  filename = api_utils._quote_filename(filename)
  status, resp_headers, content = yield api.delete_object_async(filename)
  errors.check_status(status, [204], filename, resp_headers=resp_headers,
                      body=content)


def get_location(bucket, retry_params=None):
  ...
  # Issues request of form: https://storage.googleapis.com/a-bucket?location
  # and parses response of form:
  # <LocationConstraint>US</LocationConstraint>
  return get_location_async(bucket, retry_params).get_result()


@ndb.tasklet
def get_location_async(bucket, retry_params=None, token_params=None):
  """Asynchronously returns the location for the given bucket.
  ...
  Returns:
    A Future whose get_result method returns the location as a string.
  ...
  # and parses response of form:
  # <LocationConstraint>US</LocationConstraint>
  location = yield _get_bucket_attribute_async(
      bucket,
      'location',
      'LocationConstraint',
...
def get_storage_class(bucket, retry_params=None):
  ...
  Raises:
    ...
    errors.NotFoundError: if the bucket does not exist.
  """
  # Issues request of form: https://storage.googleapis.com/a-bucket?storageClass
  # and parses response of form:
  # <StorageClass>STANDARD</StorageClass>
  return get_storage_class_async(bucket, retry_params).get_result()


@ndb.tasklet
def get_storage_class_async(bucket, retry_params=None, token_params=None):
  """Asynchronously returns the storage class for the given bucket.
  ...
  Returns:
    A Future whose get_result method returns the storage class as a string.
  ...
  # and parses response of form:
  # <StorageClass>STANDARD</StorageClass>
  storage_class = yield _get_bucket_attribute_async(
      bucket,
      'storageClass',
      'StorageClass',
      retry_params=retry_params,
      token_params=token_params)
  raise ndb.Return(storage_class)


@ndb.tasklet
def _get_bucket_attribute_async(bucket,
                                query_param,
                                xml_response_tag,
                                retry_params=None,
                                token_params=None):
  ...
  status, headers, content = yield api.get_bucket_async('%s?%s' % (bucket,
                                                                   query_param))
  ...
  if root.tag == xml_response_tag and root.text:
    raise ndb.Return(root.text)
  raise ndb.Return(None)


def stat(filename, retry_params=None):
  ...
  Raises:
    ...
    errors.NotFoundError: if an object that's expected to exist doesn't.
  """
  return stat_async(filename, retry_params).get_result()


@ndb.tasklet
def stat_async(filename, retry_params=None, token_params=None):
  """Get GCSFileStat of a Google Cloud storage file.
  ...
  api = storage_api._get_storage_api(
      retry_params=retry_params, token_params=token_params)
  status, headers, content = yield api.head_object_async(
  ...
  raise ndb.Return(file_stat)


def copy2(src, dst, metadata=None, retry_params=None, token_params=None):
...
def compose(list_of_files, destination_file, files_metadata=None,
            content_type=None, retry_params=None):
  ...
  """
  api = storage_api._get_storage_api(
      retry_params=retry_params, token_params=None)
  ...
  if full_app_id.get() == 'testbed-test':
    def _temp_func(file_list, destination_file, content_type):
      """TODO: Dev server stub remove when the dev server accepts
      compose requests.
      """
      bucket = '/' + destination_file.split('/')[1] + '/'
      with open(destination_file, 'wb', content_type=content_type) as gcs_merge:
        for source_file in file_list:
          with open(bucket + source_file['Name'], 'rb') as gcs_source:
...
def _validate_compose_list(destination_file, file_list,
  ...
  list_of_files = []
  for source_file, meta_data in six.moves.zip_longest(file_list,
                                                      files_metadata):
...
class _Bucket(object):
  ...
  def __iter__(self):
    ...
    while self._get_bucket_fut:
      ...
      # XML results have a section for files and another section for
      # dirs. Each is separately ordered. We need to merge them.
      # For examples, see
      ...
      next_file = six.next(files)
      next_dir = six.next(dirs)
  ...
  def _find_elements(self, result, elements):
    ...
    """
    element_mapping = {}
    result = six.StringIO(six.ensure_str(result))
