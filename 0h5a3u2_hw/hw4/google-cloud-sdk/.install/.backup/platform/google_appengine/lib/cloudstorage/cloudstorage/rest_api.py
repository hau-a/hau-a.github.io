...
class _RestApi(object):
  ...
  def __getstate__(self):
    ...
    return {
        'scopes': self.scopes,
        'a_maker': (
            None if self.make_token_async == _make_token_async else  # pylint: disable=comparison-with-callable
            self.make_token_async),
  ...
  def get_token_async(self, refresh=False):
    ...
    if refresh or ts is None or ts.expires < (
      ...
      token, expires_at = yield self.make_token_async(self.scopes,
                                                      self.token_params)
  ...
  def urlfetch_async(self, url, method='GET', headers=None,
    ...
    ctx = ndb.get_context()
    # TODO(user): Explicitly set validate_certificate to True or False?
