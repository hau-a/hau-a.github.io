...
calling a mock in multiple threads, it should be guarded by a mutex.
...
Migration to Python 3 targeted compatibility with 2.7 and 3.3+
"""
...
def _IsCallable(object, shallow_lookup=False):
  """Checks if an object is callable.

  Args:
    object: A python object
    shallow_lookup: (default False) If True will only look at current class
        and not inspect parent classes

  Returns:
    True if object is callable
  """
  if shallow_lookup:
    # "shallow" lookup: look presence of __call__ in first level
    return ('__call__' in dir(object) or hasattr(object, '__call__'))
  else:
    # "deep" lookup: use language provided methods
    # do not directly use the callable() builtin in py 3 or 2to3 will replace it
    # also, isinstance(object, collections.Callable) is know to have flakiness
    # in Python 3
    return (__builtins__['callable'](object) if _IS_PYTHON_3 else
            callable(object))


def _Caller(max_frames):
  """Finds caller's (filename, line), skipping any frames in mox.py.

  Args:
    max_frames: looks at most this many frames above the current position. This
        usually depends on how many calls mox.py makes between the original
        user and the internal caller to _Caller().

  Returns:
    (filename, line), or None if it cannot be determined.
  """
  # Avoid traceback module, it has many transitive dependencies, some of
  # which may be mocked. We really only need a filename:lineno.
  for depth in range(max_frames):
    f = sys._getframe(depth)
    if not f.f_code.co_filename.endswith("/mox.py"):
      return _os_path_basename(f.f_code.co_filename), f.f_lineno
  return None
...
class ExpectedMethodCallsError(Error):
  ...
  def __init__(self, expected_methods):
    ...
    Error.__init__(self, expected_methods)
    self._expected_methods = expected_methods

  def __str__(self):
    def _MaybeSourceLocation(m):
      if m.expect_location:
        return " [Expectation set at: %s:%d]" % m.expect_location
      else:
        return ""
    calls = "\n".join("%3d.  %s%s" % (i, m, _MaybeSourceLocation(m))
...
class UnexpectedMethodCallError(Error):
  ...
  def __init__(self, unexpected_method, expected):
    ...
    Error.__init__(self, unexpected_method, expected)
    if expected is None:
      ...
      chunks = ["Unexpected method call.  unexpected:-  expected:+\n"]
      if expected.expect_location:
        chunks.append("[Expectation set at: %s:%d]\n" %
                      expected.expect_location)
      chunks.append("\n".join(line.rstrip() for line in diff))
      self._str = "".join(chunks)

  def __str__(self):
    return self._str


class UnknownMethodCallError(Error, AttributeError):
  """Raised if an unknown method is requested of the mock object.

  This is also a subclass of AttributeError so that code introspecting
  a mock object using getattr() will work.
  """
  ...
  def __init__(self, unknown_method_name):
    ...
    Error.__init__(self, unknown_method_name)
...
class ExpectedMockCreationError(Error):
  ...
  def __init__(self, expected_mocks):
    ...
    if not expected_mocks:
      raise ValueError("There must be at least one expected method")
    Error.__init__(self, expected_mocks)
...
class UnexpectedMockCreationError(Error):
  ...
  def __init__(self, instance, *params, **named_params):
    ...
    # named_params cannot be passed to Error, because it doesn't support them.
    Error.__init__(self, instance, *params)
...
class SwallowedExceptionError(Error):
  ...
  def __init__(self, previous_exceptions):
    ...
    Error.__init__(self, previous_exceptions)
...
class Mox(object):
  ...
  if _IS_PYTHON_3:
    _USE_MOCK_OBJECT = [type, types.FunctionType, object,
                        types.ModuleType, types.MethodType]
  else:
    # no UnboundMethodType in Python 3
  ...
  # TODO(user): Remove this guard for the swallowed exception behaviour once
  # all google3 tests are fixed.
  RAISE_SWALLOWED_EXCEPTIONS = False
  ...
  def CreateMock(self, class_to_mock, attrs=None):
    ...
    Args:
      ...
      class_to_mock: An object of any type listed in _USE_MOCK_OBJECT
  ...
  def Verification(self):
    """Return a context handler, for use in a with statement.

    Entering the block will call ReplayAll().

    Successfully exiting the block (i.e. without an exception) will trigger
    a call to VerifyAll().

    Returns:
      A context handler object.
    """
  ...
  def StubOutWithMock(self, obj, attr_name, use_mock_anything=False):
    ...
    if (not use_mock_anything and
        (attr_type in self._USE_MOCK_OBJECT or
         (_IS_PYTHON_3 and ' object at 0x' in str(attr_to_replace)))):
      # in Python 3 a class attr_type will not be found in _USE_MOCK_OBJECT so
      # we need to check the string representation (which is a hack!)
...
class MockAnything:
  ...
  if _IS_PYTHON_3:
    ...
    def __call__(self, *args, **kwargs):
      """Intercept __call__ for the object."""
      return self._CreateMockMethod('__call__')(*args, **kwargs)

    def __getitem__(self, *args, **kwargs):
      """Intercept indexing for the object."""
      return self._CreateMockMethod('__getitem__')(*args, **kwargs)

    def __str__(self, *args, **kwargs):
      """Intercept __str__ for the object."""
      return self._CreateMockMethod('__str__')(*args, **kwargs)

  def _CreateMockMethod(
      self, method_name, method_to_mock=None, method_class=None):
  ...
  if _IS_PYTHON_3:
    ...
    def __bool__(self):
      """Always return True so the mock can be used as a conditional."""
      return True
  ...
  def _Verify(self):
    ...
    if Mox.RAISE_SWALLOWED_EXCEPTIONS and self._exceptions_thrown:
...
class MockObject(MockAnything, object):
  ...
  def __init__(self, class_to_mock, attrs=None):
    ...
    try:
      if _IS_PYTHON_3:
        if inspect.isclass(self._class_to_mock):
          ...
          self._description = class_to_mock.__qualname__
      else:
        if (inspect.isclass(self._class_to_mock) or
            inspect.isfunction(self._class_to_mock)):
          self._description = class_to_mock.__name__
        elif inspect.ismethod(self._class_to_mock):
          self._description = '%s.%s' % (class_to_mock.im_class.__name__,
                                         class_to_mock.__name__)
        else:
    ...
    # than manually setting attributes that are normally created in __init__.
  ...
  def __getattr__(self, name):
    ...
    if name in self._known_methods:
      return self._CreateMockMethod(
          ...
          method_class=self._class_to_mock)
  ...
  if _IS_PYTHON_3:
    def __hash__(self):
      """Defines a hash for MockObject.

      Needed to support putting this class in containers.
      """
      return id(self)
  ...
  def __contains__(self, key):
    ...
    """
    if '__contains__' not in dir(self._class_to_mock):
      raise TypeError('unsubscriptable object')
  ...
  def __call__(self, *params, **named_params):
    ...
    if not _IsCallable(self._class_to_mock, shallow_lookup=True):
    ...
    if type(self._class_to_mock) in (types.FunctionType, types.MethodType):
      ...
      if _IS_PYTHON_3:
        # this must be the ugliest hack ever to track down a class from a method
        method_class = (eval('global_module.' +
                             self._class_to_mock.__qualname__.rsplit('.', 1)[0],
                             {'global_module': sys.modules[
                                 self._class_to_mock.__module__]})
                        if '.' in self._class_to_mock.__qualname__ and
                        '<locals>' not in self._class_to_mock.__qualname__
                        else None)
      else:
        method_class = (self._class_to_mock.im_class
                        if 'im_class' in self._class_to_mock.__dict__ else None)
    else:
      method = getattr(self._class_to_mock, '__call__')
      method_class = self._class_to_mock
    mock_method = self._CreateMockMethod(
        '__call__', method_to_mock=method, method_class=method_class)

    return mock_method(*params, **named_params)

  # In Python 2.7, __enter__ and __exit__ are not looked up on the instance
  # but on the class (like most special methods.)
  def __enter__(self):
    """Provide custom logic for mocking classes that are context managers.
    ...
    Returns:
      Expected return value in replay mode.  A MockMethod object for the
      __enters__ method that has already been called if not in replay mode.

    Raises:
      AttributeError if the underlying class does not implement __enter__.
      UnexpectedMethodCaller if the object does not expect the call to
      __enter__.

    """
    enter = getattr(self._class_to_mock, '__enter__', None)

    if enter is None:
      raise AttributeError('__enter__')

    if self._replay_mode:
      return MockMethod('__enter__', self._expected_calls_queue,
                        self._exceptions_thrown, self._replay_mode)()

    return self._CreateMockMethod('__enter__')()

  def __exit__(self, type, value, traceback):
    """Provide custom logic for mocking classes that are context managers.

    Args:
      type: The type of the exception raised in the with block, or None.
      value: The exception instance raised in the with block, or None.
      traceback: The traceback for the exception, or None.

    Returns:
      Expected return value in replay mode.  A MockMethod object for the
      __exits__ method that has already been called if not in replay mode.

    Raises:
      AttributeError if the underlying class does not implement __exit__.
      UnexpectedMethodCaller if the object does not expect the call to
      __exit__.

    """
    exit = getattr(self._class_to_mock, '__exit__', None)

    if exit is None:
      raise AttributeError('__exit__')

    if self._replay_mode:
      return MockMethod('__exit__', self._expected_calls_queue,
                        self._exceptions_thrown, self._replay_mode)(
                            type, value, traceback)

    return self._CreateMockMethod('__exit__')(type, value, traceback)
...
class _MockObjectFactory(MockObject):
  ...
  def __call__(self, *params, **named_params):
    ...
    mock_method = self._CreateMockMethod(
        '__init__', method_to_mock=method, method_class=self._class_to_mock)
...
class MethodSignatureChecker(object):
  ...
  _NEEDED, _DEFAULT, _GIVEN = (0, 1, 2)

  def __init__(self, method, class_obj=None):
    """Creates a checker.

    Args:
      # method: A method to check.
      method: function
      class_obj: (default None) class for method, if any
    ...
    # In Python 2, these types of objects would have raised TypeError from
    # inspect.getargspec above.
    # >>> class A: pass
    # ...
    # >>> A.__call__
    # <method-wrapper '__call__' of type object at 0x7f9e9b414038>
    if type(method).__name__ == 'method-wrapper':
      raise ValueError('Disabling signature checking for method-wrapper '
                       '(compatibility with Python 2).')
    self._method = method
    self._class = class_obj
    if _IS_PYTHON_3:
      self._isbound = (((inspect.isfunction(method)
                         and not inspect.ismodule(self._class)) or
                        inspect.ismethod(method) or
                        inspect.ismethoddescriptor(method))
                       and self._class is not None)
    else:
      self._isbound = inspect.ismethod(method)
    if self._isbound:
  ...
  def Check(self, params, named_params):
    ...
    if self._isbound:
      ...
      if len(params) > len(self._required_args):
        expected = (self._class if _IS_PYTHON_3 and self._class is not None else
                    getattr(self._method, 'im_class', None))
...
class MockMethod(object):
  ...
  def __init__(self, method_name, call_queue, exception_list, replay_mode,
               method_to_mock=None, description=None, method_class=None):
    ...
    Args:
      ...
      method_class: (default None) class for method object, if any
    """
    ...
    try:
      self._checker = MethodSignatureChecker(
          method_to_mock, class_obj=method_class)
    except ValueError:
      self._checker = None
  ...
  def __call__(self, *params, **named_params):
    ...
    if not self._replay_mode:
      self.expect_location = _Caller(5)
  ...
  def __str__(self):
    ...
    if self._description:
      if full_desc.startswith('__call__('):
        full_desc = full_desc.lstrip('__call__')
  ...
  def __hash__(self):
    """Defines a hash for MockMethod.

    Needed to support putting this class in containers, which is needed
    by the MultipleTimesGroup class.
    """
    return hash('%r-%r-%r' % (self._name, self._params, self._named_params))
...
class Regex(Comparator):
  ...
  def __repr__(self):
    ...
    if (self.regex.flags and
        not (_IS_PYTHON_3 and self.regex.flags == re.UNICODE)):
      # in PY3 strings are always unicode, so re.UNICODE is always implied
...
class _AlwaysComparableWrapper(object):
  """Wraper for any object that tries to be always comparable (and sortable)."""
  ...
  def __lt__(self, other):
    """Makes class always sortable for use in SameElementsAs().

    Args:
      other: The other object to compare to (expects _AlwaysComparableWrapper)
    """
    return str(self) < str(other)

  def __eq__(self, other):
    """Defines a comparison by the internal 'real' value.
  ...
  def __repr__(self):
    """Defines a string representation: helps with debugging."""
  ...
  def equals(self, actual_seq):
    ...
    try:
      ...
      try:
        ...
        # not all lists are comparable in Py 3: convert to a comparable wrapper
        expected = [_AlwaysComparableWrapper(x) for x in expected]
        actual = [_AlwaysComparableWrapper(x) for x in actual]
        expected.sort()
        actual.sort()
...
class MethodGroup(object):
  ...
  def __init__(self, group_name, exception_list):
    ...
    self._exception_list = exception_list
    self.expect_location = _Caller(7)
...
class VerificationHandler(object):
  """Helper class to allow verification of a context handler block."""
...
class MoxMetaTestBase(type):
  ...
  def __init__(cls, name, bases, d):
    ...
    for func_name, func in d.items():
      if func_name.startswith('test') and _IsCallable(func):
...
class MoxTestBase(unittest.TestCase):
  ...
  __metaclass__ = MoxMetaTestBase  # keep because python 3 will ignore


def _MoxTestBaseSetUp(mox_self):
  """Function that will be replaced into the MoxTestBase.setUp() method."""
  super(MoxTestBase, mox_self).setUp()
  mox_self.mox = Mox()
  mox_self.stubs = stubout.StubOutForTesting()


# the v.2 syntax for metaclasses use the __metaclass__ declaration
# the v.3 syntax for metaclasses use the metaclass= parameter at class creation
# the two are incompatible across versions: below is the version-compatible hack
MoxTestBase = MoxMetaTestBase(
    'MoxTestBase',
    (unittest.TestCase,),  # base(s) for MoxTestBase
    dict(setUp=_MoxTestBaseSetUp,
         __module__=MoxTestBase.__module__,
